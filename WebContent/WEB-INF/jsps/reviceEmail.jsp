...
<body>
					...
					<div class="pages">
						<table width="90%" border="0" cellspacing="0" cellpadding="0">
							...
							<c:forEach items="${findEmailByName }" var="byname">
								<tr>
									<td align="left"><a
										href="${pageContext.request.contextPath}/user/emailinfor?email_id=${byname.email_id}">
											${byname.email_title}</a></td>
									<td align="left">${byname.email_body}</td>
									<c:if test="${byname.isRead == 0}">
										<td align="left">未读</td>
									</c:if>
									<c:if test="${byname.isRead == 1}">
										<td align="left">已读</td>
									</c:if>
									<td align="left">${byname.sendTime}</td>
									<td><a href="javascript:void(0);"
										onclick="deleteEmail('${byname.email_id}')">删除</a></td>
								</tr>
							</c:forEach>
						</table>
	...
	<script type="text/javascript">
		function deleteEmail(email_id) {
			var con = confirm("确定放入回收站?"); //在页面上弹出对话框
			if (con == true) {
				location.href = "${pageContext.request.contextPath}/user/logicallydelete?email_id="
						+ email_id;
			}
		}
	</script>
